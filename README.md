...
- [`signet`](https://github.com/go-signet/signet) — Authorization Server (Auth Code + PKCE, Device Grant, Client Credentials; OIDC Core; MCP-ready with RFC 8707 / RFC 8414)
- [`cli`](https://github.com/go-signet/cli) — Smart OAuth CLI that auto-selects the right flow for the runtime environment
- [`sdk-go`](https://github.com/go-signet/sdk-go) / [`sdk-python`](https://github.com/go-signet/sdk-python) — SDKs for token acquisition, secure storage, and JWT validation
- [`helm-signet`](https://github.com/go-signet/helm-signet) — Helm chart for single-instance (SQLite) or HA (PostgreSQL + Redis) Kubernetes deployment
- [`examples`](https://github.com/go-signet/examples) — Multi-language usage examples (Go, Python, Bash)
